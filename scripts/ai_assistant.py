...
def get_system_info():
    ...
    try:
        ...
        info['uptime'] = 'N/A'
    try:
        info['disk'] = subprocess.check_output(['df', '-h', '/'], text=True).strip()
    except Exception:
        info['disk'] = 'N/A'
    try:
        info['memory'] = subprocess.check_output(['free', '-h'], text=True).strip()
    except Exception:
        info['memory'] = 'N/A'
    try:
        info['docker_ps'] = subprocess.check_output(['docker', 'ps', '--format', '{{.Names}}: {{.Status}}'], text=True).strip()
    except Exception:
        info['docker_ps'] = 'N/A'
    try:
        info['docker_images'] = subprocess.check_output(['docker', 'images', '--format', '{{.Repository}}:{{.Tag}}'], text=True).strip()
    except Exception:
        info['docker_images'] = 'N/A'
    try:
        info['load'] = subprocess.check_output(['cat', '/proc/loadavg'], text=True).strip()
    except Exception:
        info['load'] = 'N/A'
    return info
...
def get_recent_logs(service_name, lines=50):
    ...
    try:
        logs = subprocess.check_output(['docker', 'logs', '--tail', str(lines), service_name], text=True, stderr=subprocess.STDOUT)
        return logs[-4000:]
    except Exception as e:
        return f'Could not fetch logs: {e}'

def get_health_check_logs():
    """Get recent health check logs from EC2."""
...
def build_system_context():
    """Build a comprehensive context string about the environment."""
    sys_info = get_system_info()
    health = {}
    for svc in ['sample_app', 'nginx', 'prometheus', 'grafana']:
        health[svc] = check_service_health(svc)
    return f"""You are an AI assistant for the Auto DevOps Lab. Here is the current environment state:

--- SYSTEM INFO ---
Uptime: {sys_info['uptime']}
Load Average: {sys_info['load']}
Disk: {sys_info['disk']}
...
Respond in plain English, be concise but thorough. Suggest actionable next steps."""

@app.route('/ai/explain', methods=['GET', 'POST'])
def explain():
    """Explain the current environment state."""
    ...
    messages = [
        ...
        {'role': 'user', 'content': f"Explain the current state of this Auto DevOps Lab environment. {user_question}".strip()}
    ]
...
def troubleshoot():
    ...
    messages = [
        {'role': 'system', 'content': f"{context}\n\nRecent logs for '{service}' (if applicable):\n{logs[:2000]}\n\nYou are a DevOps troubleshooting expert. Analyze the issue and provide a diagnosis and fix."},
...
def analyze_logs():
    ...
    context = build_system_context()
    messages = [
        {'role': 'system', 'content': f"{context}\n\nYou are a log analysis expert. Find errors, warnings, and anomalies in the following logs and explain what they mean."},
